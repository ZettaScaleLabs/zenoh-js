...
EMSCRIPTEN_KEEPALIVE
int version() {
    return Z_PROTO_VERSION;
}

EMSCRIPTEN_KEEPALIVE
void* default_config(const char *locator) {
    if (locator == NULL) {
            return NULL;
    }

    z_owned_config_t *config = (z_owned_config_t*) z_malloc(sizeof(z_owned_config_t));
...
EMSCRIPTEN_KEEPALIVE
void* open_session(z_owned_config_t *config) {
...
EMSCRIPTEN_KEEPALIVE
int start_tasks(z_owned_session_t *s) {
...
EMSCRIPTEN_KEEPALIVE
void* declare_ke(z_owned_session_t *s, const char *keyexpr){
    z_owned_keyexpr_t *ke =  (z_owned_keyexpr_t*) z_malloc(sizeof(z_owned_keyexpr_t));
...
EMSCRIPTEN_KEEPALIVE
int pub(z_owned_session_t *s, z_owned_keyexpr_t *ke, char* value){
    z_put_options_t options = z_put_options_default();
    options.encoding = z_encoding(Z_ENCODING_PREFIX_TEXT_PLAIN, NULL);
    return z_put(z_loan(*s), z_loan(*ke), (const uint8_t *)value, strlen(value), &options);
}
...
EMSCRIPTEN_KEEPALIVE
void close_session(z_owned_session_t *s){
    z_close(z_move(*s));
}
...
void* sub(z_owned_session_t *s, z_owned_keyexpr_t *ke, int js_callback) {
    ...
    z_owned_closure_sample_t callback = z_closure(wrapping_sub_callback, NULL, (void*) js_callback);
...
EMSCRIPTEN_KEEPALIVE void call_js_function_on_another_thread(int js_callback_id) {
